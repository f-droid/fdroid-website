...
This Anti-Feature is applied to apps that promote or depend entirely on a proprietary
network service.
...
This Anti-Feature is applied to apps that depend entirely on a service which is impossible
(or not easy) to replace.
Replacement requires changes to the app or service. This Anti-Feature would not be applied
if there is a simple configuration option that allows pointing the app to a running
instance of an alternative, publicly available, self-hostable server solution.

Here's the list of [apps with Tethered Network Services](https://monitor.f-droid.org/anti-feature/TetheredNet).
