...
Whenever F-Droid has built a new index, it proactively pushes it to a selected set of mirrors, called the "primary mirrors". In other words, primary mirrors are updated nearly immediately. The primary mirrors are configured in ([latest list](https://gitlab.com/fdroid/web-services/-/blob/master/origin-04-mirror-push.yml)):

* ftp.agdsn.de
* ftp.lysator.liu.se
* plug-mirror.rcac.purdue.edu

The other mirrors are "secondary mirrors".  These mirrors should sync periodically (e.g., every 4 hours) from one of the primary mirrors. Therefore, secondary mirrors usually have a small propagation delay.
Nevertheless, note that in practice the time between any two index updates is dominated by the time to update and build the apps (in the order of days). Limiting the number of primary mirrors keeps the load on the _f-droid.org originserver_ small.  It is possible to have a "tertiary mirror" by syncing from a secondary mirror, which could make sense given local bandwidth constraints.
...
Running a mirror mostly requires lots of disk space and bandwidth. Bandwidth requirements are reduced with each new mirror, but disk requirements grow at a [reasonable rate](https://ftp.fau.de/cgi-bin/show-ftp-stats.cgi?statstype=2&what=mirrorsize&mirrorname=fdroid&timespan=-1&graphsize=large&submit=Go%21).
...
* Fork the [mirror monitor repo](https://gitlab.com/fdroid/mirror-monitor), add your mirror to the list in the _README_, and open a merge request.
